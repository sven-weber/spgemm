...
partition::Partitions balanced_partition(matrix::CSRMatrix<> &C, int mpi_size) {
  ...
  for (size_t row = 0; row < C.height; row++) {
    ...
    if(non_zero_count > non_zeros_per_partition) {
      std::cout << "Rows: " << row - p[node].start_row << " - Non Zero: " << non_zero_count << std::endl;
      non_zero_count = 0;
      p[node].end_row = row;
      p[node+1].start_row = row;
      node++;

      if(node == mpi_size) break;
    }
  }
  std::cout << "Rows: " << C.height - p[node].start_row << " - Non Zero: " << non_zero_count << std::endl;
