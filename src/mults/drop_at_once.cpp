...
DropAtOnce::DropAtOnce(int rank, int n_nodes, partition::Partitions partitions,
...
void DropAtOnce::save_result(std::string path) {
  matrix::ManagedCSRMatrix result(cells);
  result.save(path);
}

size_t DropAtOnce::get_B_serialization_size() {
