...
unsigned long serialize_time_to_unsigned_long(const std::chrono::system_clock::time_point& time_point) {
  // Convert time_point to milliseconds since the Unix epoch and cast to unsigned long
  return static_cast<unsigned long>(std::chrono::duration_cast<std::chrono::milliseconds>(time_point.time_since_epoch()).count());
}

std::chrono::system_clock::time_point deserialize_time_from_unsigned_long(unsigned long time_as_ulong) {
...
void sync_start_time(int rank) {
  ...
  MPI_Bcast(&time_to_wait_for, sizeof(time_to_wait_for), MPI_BYTE, MPI_ROOT_ID, MPI_COMM_WORLD);
